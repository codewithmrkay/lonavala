# 🚀AI-Powered Trip Planner

[![Vercel](https://img.shields.io/badge/Hosted%20on-Vercel-black?logo=vercel&style=flat)](https://post-vu47.vercel.app)
[![Next.js](https://img.shields.io/badge/Next.js-15-blue?logo=next.js)](https://nextjs.org/)
[![Tailwind](https://img.shields.io/badge/TailwindCSS-white?logo=tailwindcss)](https://tailwindcss.com/)
[![shadcn/ui](https://img.shields.io/badge/UI-shadcn/ui-purple?logo=storybook)](https://ui.shadcn.dev/)
[![MongoDB](https://img.shields.io/badge/MongoDB-Atlas-green?logo=mongodb)](https://mongodb.com)
[![AI Powered](https://img.shields.io/badge/AI-Gemini-blueviolet?logo=google)](https://makersuite.google.com/)
[![License: MIT](https://img.shields.io/badge/license-MIT-green)](LICENSE)

A modern trip planning app built with **React.js**, **FireBase**, and **Gemini AI**, empowering travelers with tailored itineraries—from solo adventures to group journeys. Enjoy personalized travel plans featuring budget-friendly hotel recommendations, curated points of interest, and robust secure authentication.

---

## Table of Contents

- 🌐[Live Demo](#live-demo)
- ✨[Features](#features)
- 🛠[Tech Stack](#tech-stack)
- 📥[Installation](#installation)
- 📞[Contact](#contact)

## 🌐Live Demo

🔗 [Visit the live app on Vercel](https://lonavala-alpha.vercel.app)

---

## ✨Features

- 🛫 **Travel for Everyone:** Plan trips perfectly tailored from solo adventures to group journeys.  
- 🎯 **Point of Interest Recommendations:** Discover the best must-visit spots at every destination.  
- 🏨 **Budget-friendly Hotel Picks:** Get curated hotel suggestions that match your budget.  
- 🗺️ **Personalized Itineraries:** Enjoy customized travel plans that suit your style and needs.  
- 🔐 **Secure Access:** Benefit from robust authentication to keep your travel data safe.

---

## 🛠Tech Stack

| Frontend | Backend    | AI         | Database       | Hosting |
|----------|------------|------------|----------------|---------|
| React.js  | API Routes | Gemini API | Firebase Studio  | Vercel  |

---

## 📞Contact

<p align="center"> 
  <a href="mailto:kkadam58707@gmail.com" target="_blank"> 
    <img src="https://img.shields.io/badge/Email-kkadam58707@gmail.com-800080?style=for-the-badge&logo=gmail&logoColor=white" alt="Email"> 
  </a> 
 <a href="https://www.youtube.com/@Code_Gully" target="_blank"> 
  <img src="https://img.shields.io/badge/YouTube-Code_Gully-FF0000?style=for-the-badge&logo=youtube&logoColor=white" alt="YouTube"> 
 </a> 
 <a href="https://www.linkedin.com/in/karan-kadam-777621366/" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-Karan%20Kadam-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"> 
 </a> 
</p>

---

## 📥Installation

### Requirements

- React.js ≥ 18  
- FireBase URI  (get from FireBase Studio)  
- Gemini API Key (get from Google AI Studio)  

### 💻Setup

```bash
git clone https://github.com/codewithmrkay/lonavala.git
cd lonavala
npm install
